  ...
  <div class="home-container">
    <z-row :gutter="15">
      ...
      <z-col :span="10">
        ...
        <select v-model="types" placeholder="请选择查询类型" style="margin:0;height:20px; background: none;border-radius: 30px; border: 1px solid #9194a7; color: #2885c0;" @change="upDateType">
          <option v-for="item in typeData" :key="item" :label="item" :value="item" style="background-color: #2885c0"></option>
        </select>
...
export default {
  ...
  methods: {
    ...
    upDateType() {
      setCommitType(this.types)
    }
  },
  setup(props, context) {
    const { year, sum, types } = useResize();
